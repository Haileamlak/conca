...
func (p *PostgresStore) GetHistory(brandID string) ([]models.Post, error) {
	...
	for rows.Next() {
		...
		var status string
		err := rows.Scan(
			&post.ID, &post.SocialID, &post.BrandID, &post.Topic, &post.Content,
...
func (p *PostgresStore) SaveBrand(brand models.BrandProfile, userID string) error {
	query := `
		INSERT INTO brands (id, user_id, name, industry, voice, target_audience, topics, anti_topics)
	...
	_, err := p.pool.Exec(context.Background(), query,
		brand.ID, userID, brand.Name, brand.Industry, brand.Voice, brand.TargetAudience, topicsJSON, antiTopicsJSON,
	)
	return err
}

func (p *PostgresStore) GetBrand(id string) (models.BrandProfile, string, error) {
	query := `SELECT id, user_id, name, industry, voice, target_audience, topics, anti_topics FROM brands WHERE id = $1`
	var brand models.BrandProfile
	var userID string
	var topics, antiTopics []byte

	err := p.pool.QueryRow(context.Background(), query, id).Scan(
		&brand.ID, &userID, &brand.Name, &brand.Industry, &brand.Voice, &brand.TargetAudience, &topics, &antiTopics,
	)
	if err != nil {
		return brand, "", err
	}
...
func (p *PostgresStore) ListBrands(userID string) ([]models.BrandProfile, error) {
	query := `SELECT id, name, industry, voice, target_audience, topics, anti_topics FROM brands WHERE user_id = $1`
	...
	for rows.Next() {
		...
		var topics, antiTopics []byte
		err := rows.Scan(&b.ID, &b.Name, &b.Industry, &b.Voice, &b.TargetAudience, &topics, &antiTopics)
		if err != nil {
			return nil, err
		}
